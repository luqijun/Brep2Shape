...
class FinetuningDataset(Dataset):
    """Load processed B-rep geometry for classification or segmentation."""
    ...
    def __init__(
        ...
        self.use_for_classification = use_for_classification
    ...
    def _load_split(self, split: str) -> list[dict[str, Any]]:
        split_path = self.path / "datasplit_new.json"
    ...
    def load_samples(
        ...
        if num_workers > 0:
            ...
            with ThreadPoolExecutor(max_workers=num_workers) as executor:
                ...
                iterator = zip(
                    items,
                    tqdm(results, total=len(items), desc="Loading samples"),
                )
                for index, (item, result) in enumerate(iterator):
                    sample, error = result
                    if sample is not None and error == "success":
                        self.data.append(sample)
                    else:
                        invalid_samples.append((index, item, error))
        else:
            for index, item in enumerate(tqdm(items, desc="Loading samples")):
                ...
                with error_file.open("w", encoding="utf-8") as f:
                    json.dump(
                        [
                            ...
                            for index, item, error in invalid_samples
                        ],
                        f,
                        indent=2,
                        ensure_ascii=False,
    ...
    @staticmethod
    def _load_label(value: Any) -> torch.Tensor:
        if isinstance(value, str):
            raw_label = np.loadtxt(value)
        elif isinstance(value, (int, np.integer, list, tuple, np.ndarray)):
            raw_label = value
    ...
    def load_one_sample(self, item):
        """Load and validate one processed fine-tuning sample."""
        ...
        label = self._load_label(item["label"])
        ...
        data["file_name"] = item["face"]
        return data

    def load_face(self, file_path):
        """Load face primitives and face-level UV targets."""
        # Legacy processed files contain Python and NumPy objects, so they
    ...
    def load_topo(self, file_path):
        """Load edge primitives, topology indices, and edge-level UV targets."""
        # See load_face for why trusted legacy data uses weights_only=False.
